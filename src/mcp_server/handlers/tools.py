...
class ToolHandler:
    ...
    async def _bus_route_tool(self, args: Dict[str, Any]) -> List[TextContent]:
        ...
        if route_key in self.bus_routes:
            ...
            return [TextContent(type="text", text=f"No direct bus route found from {from_loc} to {to_loc}.")]
    ...
    async def _whatsapp_bus_query_tool(self, args: Dict[str, Any]) -> List[TextContent]:
        ...
        if not parsed:
            return [TextContent(type="text", text="I couldn't understand your request. Please ask like: 'I want to go from banashankari to majestic'")]
